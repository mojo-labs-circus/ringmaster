...
> **Session start:** Read `CLAUDE.md` (this file), `jarvis-dev-context.md`, and `jarvis-testing-context.md` before doing anything else. The spec lives in `circus/spec/ringmaster/` — read only the files relevant to the current task.

## Current Task
Mk1 — SQLite repository integration tests and auth routes integration tests complete. Next: `test_profile_routes.py`, `test_chat_ws.py`, `test_cleanup.py`, then ORCHESTRATOR node.
