...
| 2026-04-13 | Spec audit complete — rev 26. Resolved three DAG orchestration ambiguities: `intent` changed to `list[str]`, `Step` and `StepResult` TypedDicts defined, ORCHESTRATOR owns error handling for plan-step failures (not graph edges). `state.py` and `chat.py` updated to match. Full codebase audit: five issues found and fixed — `create_tables()` now called from FastAPI lifespan (sqlite only), Phase 1/2 node files deleted, `PostgresAuthRepository` missing `update_assistant_name` stub added, stale TODO removed from `db/history/repository.py`. |
