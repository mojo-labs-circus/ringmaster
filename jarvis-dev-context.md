...
- **JARVIS code:** `~/projects/circus/MojOS/ringmaster/`
...
| 2026-05-02 (7) | Naming conventions pass — full codebase audit. HistoryRepository.load() return type fixed from list[dict] to list[HistoryEntry]; budget logic and dict conversion moved to tools/history.py. _row_to_entry() helper added to db/history/sqlite.py; _row_to_task() restored and moved before class in db/tasks/sqlite.py. memory/ deleted (relic of old version, to be rebuilt when MEMORY nodes are implemented). All 58 unit tests pass. |
| 2026-05-03 (1) | Manual graph output test scripts — test_node_prompt_engineer.py, test_node_router.py, test_node_planner.py, test_graph_flow.py. All support -m flag for custom messages. All three nodes verified working correctly against real Ollama. Router prompt tightened: web intent now requires live/real-time data, conversation gains explicit general knowledge signal, memory scoped to previously stored personal information. |
| 2026-05-04 (1) | Integration tests complete for all three SQLite repositories (auth, history, tasks) and auth routes. test_auth_routes.py: 27 tests covering login, refresh, logout, invite, register, reset, and change-password. Fixtures refactored — repo, admin_user, standard_user, admin_token, standard_login, invite_token, reset_token replace per-test repo construction and the old admin_client tuple. |
