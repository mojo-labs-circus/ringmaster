...
- [ ] FastAPI uses `astream_events` — sends node-entry `status` frames driven by `STATUS_MESSAGES` dict. Forwards mid-node `status_message` updates from state as `status` frames. Sends `"One moment..."` status frame immediately when a message is buffered during an active invocation (depth-1, last message wins). Rejects non-confirm/cancel messages during active interrupt with `"Please confirm or cancel the pending action first."` status frame. On `token_version` mismatch, clears the buffer and sends an `error` frame indicating re-authentication was required.
- [ ] WebSocket streaming — typed JSON frames (`token`, `done`, `error`, `status`) with `message_id`. FastAPI owns all frame sending. One invocation at a time per connection — one message may be buffered while active; processed automatically when the invocation completes.
...
- [ ] CONVERSATION node — `graph/nodes/conversation.py`. General chat for all tiers. Calls `tools/llm.py` with `messages`, `retrieved_context`, and `skill_context`. Writes to `step_response`.
...
- [ ] ROUTER updated — `needs_memory` flag per intent controls retrieval only (conditional on request type for `tasks` — see ROUTER spec), skills check with graceful no-op `[pearlybaker only for skills check]`, sets `tier_gate` list for any tier-gated intents
- [ ] RESPONDER updated — reads `client_type` from state, checks `error` field, derives and sets `refresh` list on state from `intent` (RESPONDER is sole owner of `refresh`), formats tier-gate step results using hardcoded per-capability messages from `config.yaml`
- [ ] CONSTITUTIONAL check — `api/constitutional.py` — concurrent async coroutine launched by `chat.py` when token streaming begins. Watches token buffer in real time, fires `truncate` frame on ethics violation, streams corrected continuation. Silent on happy path. Uses `constitutional` model from `config.yaml`. Admin tier sees check result as status message; Power and Standard see nothing.
